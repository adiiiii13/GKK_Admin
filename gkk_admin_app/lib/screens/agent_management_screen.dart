...
class _AgentManagementScreenState extends State<AgentManagementScreen> {
  ...
  Future<void> _loadAgents() async {
    try {
      ...
      // ⚡ Bolt: Extract unique agent IDs to perform a batch query
      // Eliminates O(N) iterative single-row requests.
      ...
      for (var ticket in tickets) {
        ...
        if (agentId != null && !uniqueAgents.containsKey(agentId)) {
          final email = ticket['agent_email'];
          uniqueAgents[agentId] = {
            'id': agentId,
            'email': email, // Might be null here, will resolve in batch query
            'is_banned': false, // Default value
          };
          agentIdsToFetch.add(agentId.toString());
        }
      }

      // Batch query for agent details (email and ban status)
      if (agentIdsToFetch.isNotEmpty) {
        try {
          ...
          for (var agentData in agentsData) {
            final agentId = agentData['id']?.toString();
            if (agentId != null && uniqueAgents.containsKey(agentId)) {
              // Only override email if we didn't get it from the ticket
              if (uniqueAgents[agentId]!['email'] == null &&
                  agentData['email'] != null) {
                uniqueAgents[agentId]!['email'] = agentData['email'];
              }
          ...
          debugPrint('Error batch fetching agent details: $e');
        }
      }

      // Finalize display names and emails
      for (var agentId in uniqueAgents.keys) {
        final email = uniqueAgents[agentId]!['email'];
        final displayEmail =
